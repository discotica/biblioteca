<script lang="ts">
  ...
  // Render progresivo para artistas grandes (sin paginacion)
  const LOTE = 36;
  let visibles = LOTE;
  ...
  $: lanzamientosOrdenados = (() => {
    switch (ordenSeleccionado) {
      case 'fecha-desc':
        return [...lanzamientos].sort((a, b) => parseReleaseDate(b.release_date).time - parseReleaseDate(a.release_date).time);
      case 'fecha-asc':
        return [...lanzamientos].sort((a, b) => parseReleaseDate(a.release_date).time - parseReleaseDate(b.release_date).time);
      case 'titulo-asc':
...
<svelte:head>
  <title>{nombreArtista} | Discótica</title>
</svelte:head>
...
<main class="main">
  <div class="container">
    ...
    <div class="cabecera-artista">
      ...
      <h1 class="nombre-artista">{nombreArtista}</h1>
    </div>

    {#if lanzamientosOrdenados.length === 0}
      <p class="mensaje-vacio">Este artista aún no tiene lanzamientos registrados en Discótica.</p>
    {:else}
